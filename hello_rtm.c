...
static void __on_rtm_data(const char *user_id, const void *data, size_t data_len)
{
    ...
    // data transfer to uint32_t, with the last 8 Bytes
    uint64_t nowtime = util_get_time_ms();
    uint64_t oldtime = 0;
    uint64_t difftime = 0;

    // sequence
    tmp[0] = ((char *)data)[0] & 0xff;
    tmp[1] = ((char *)data)[1] & 0xff;
    seq1 -= ((tmp[0] << 8)) + tmp[1];

    // oldtime
    tmp[2] = ((char *)data)[2] & 0xff;
    tmp[3] = ((char *)data)[3] & 0xff;
    tmp[4] = ((char *)data)[4] & 0xff;
    tmp[5] = ((char *)data)[5] & 0xff;
    tmp[6] = ((char *)data)[6] & 0xff;
    tmp[7] = ((char *)data)[7] & 0xff;
    tmp[8] = ((char *)data)[8] & 0xff;
    tmp[9] = ((char *)data)[9] & 0xff;

    oldtime = (tmp[2] << 56) + (tmp[3] << 48) + (tmp[4] << 40) + (tmp[5] << 32) + (tmp[6] << 24) + (tmp[7] << 16) + (tmp[8] << 8) + tmp[9];

    difftime = nowtime - oldtime;
    printf("received: seq=%u, oldtime=%lu, nowtime=%lu, difftime=%lu\n", seq1, oldtime, nowtime, difftime);
...
static void send_text_proc(app_t *p_app)
{
    ...
    uint8_t tmp[11] = {0};
    seq++;
    // put the seq in the first 2 bytes of tmp
    tmp[0] = (seq >> 8) & 0xff;
    tmp[1] = seq & 0xff;
    
    // put the time in the last 8 bytes of tmp
    tmp[2] = (now >> 56) & 0xff;
    tmp[3] = (now >> 48) & 0xff;
    tmp[4] = (now >> 40) & 0xff;
    tmp[5] = (now >> 32) & 0xff;
    ...
    rval = agora_rtc_send_rtm_data(p_app->config.p_peer_uid, tmp, 11, ++msg_id);
    
    if (rval != 0)
    {
        printf("send text failed, msg_id=%u err=%d/%s\n", msg_id, rval, agora_rtc_err_2_str(rval));
    }
    else
    {
        printf("send text success, msg_id=%u  seq=%u  stime=%lu\n", msg_id, seq, now);
    }
